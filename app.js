...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadHistory = async () => {
        ...
        try {
            ...
            allSessions.reverse().forEach(session => {
                ...
                el.querySelector('.delete-btn').addEventListener('click', () => deleteSession(session.id, el));
            });
    ...
    const loadBodyweight = async () => {
        bodyweightHistoryList.innerHTML = '<ul><li>Loading...</li></ul>';
        try {
    ...
    const loadInitialData = async () => {
        ...
        if (snapshot.exists()) {
            ...
            names.forEach(name => { exerciseDataList.innerHTML += `<option value="${name}"></option>`; });
        }
    };
